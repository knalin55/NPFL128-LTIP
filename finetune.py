...
def get_parser():
    ...
    parser.add_argument("--batch_size", default=2, type=int,
                        help="Batch size.")
    parser.add_argument("--finetune_type", default=None, type=str,
                        help="Pretraining method")
    parser.add_argument("--pretrain_type", default="mt5_wmt", type=str,
                        help="Pretraining method")
    ...
    parser.add_argument("--lang", default="ru", type=str,
                        help="Language (Relevant for monolingual finetuning and choosing dev set)")
...
def process_triples(fil_dir, lang="en"):
    ...
    for entry in entry_list:
        ...
        # entry.lexs[i].lex : text (for ru dataset,
        #                           text \in ru if i is odd else en)
        ...
        for text in entry.lexs:
            ...
            if (lang == "ru" and lang_ % 2 == 1) or lang == "en":
                for triple in entry.list_triples():
                    ...
                    inputs = text.lex
                    label = label + " " + sub + " " + " ".join(extract_component(triple, 0)) \
                                    + " " + pre + " " + " ".join([word.lower() for word in camel_case_split(" ".join(extract_component(triple, 1)))]) + " " \
                                        + obj + " " + " ".join(extract_component(triple, 2))
