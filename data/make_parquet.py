...
def get_parser():
    ...
    parser.add_argument("--directory", default="", type=str,
                        help="directory where wiki dump is located")
    parser.add_argument("--num_lines", default=500000, type=int,
                        help="Number of lines to keep in dataset")
    return parser
...
def main(args: argparse.Namespace) -> None:
    ...
    count = 0
    with open(f'{args.language}.tsv', "w") as file:
    # write n_lines to (en/ru).tsv (tmp)
        with open(os.path.join(args.directory, '{}.txt'.format(args.language))) as r_file:
        # read (en/ru) dataset file
